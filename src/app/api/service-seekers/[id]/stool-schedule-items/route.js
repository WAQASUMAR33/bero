...
export async function GET(request, { params }) {
  try {
    ...
    const scheduleItems = await prisma.serviceSeekerStoolSchedule.findMany({
    ...
    console.error('GET stool-schedule-items error:', e);
...
export async function POST(request, { params }) {
  try {
    ...
    const created = await prisma.serviceSeekerStoolSchedule.create({
      data: {
        serviceSeekerId,
        times: b.times,
    ...
    console.error('POST stool-schedule-items error:', e);
...
export async function PUT(request, { params }) {
  try {
    ...
    const updated = await prisma.serviceSeekerStoolSchedule.update({
      where: { id },
      data: {
        times: b.times,
        frequency: b.frequency,
    ...
    console.error('PUT stool-schedule-items error:', e);
...
export async function DELETE(request, { params }) {
  try {
    ...
    await prisma.serviceSeekerStoolSchedule.delete({ where: { id } });
    return NextResponse.json({ message: 'Deleted' }, { status: 200 });
  } catch (e) {
    console.error('DELETE stool-schedule-items error:', e);
