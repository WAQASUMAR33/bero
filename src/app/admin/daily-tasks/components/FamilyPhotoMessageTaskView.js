...
export default function FamilyPhotoMessageTaskView({ task, onClose }) {
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* Right Column */}
        <div className="space-y-3">
          {task.messageFromResidence && (
            <div className="bg-gray-50 rounded-xl p-3">
              <p className="text-xs font-medium text-gray-500 uppercase mb-1">Message from Residence</p>
              <p className="text-sm text-gray-900">{task.messageFromResidence}</p>
            </div>
          )}

          {task.photoUrl && (
            <div className="bg-gray-50 rounded-xl p-3">
              <p className="text-xs font-medium text-gray-500 uppercase mb-1">Photo</p>
              <img src={task.photoUrl} alt="Family photo" className="w-full h-48 object-cover rounded-lg mt-2" />
            </div>
          )}
        </div>
      </div>
