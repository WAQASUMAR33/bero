...
export async function PUT(request, { params }) {
  try {
    ...
    const existingTask = await prisma.oneToOneTask.findUnique({
      where: { id: taskId },
      include: {
        serviceSeeker: true
      }
    ...
    if (isCareWorker) {
      // Care workers can only update: task data, notes, emotion (one-to-one tasks don't have completed field)
