...
export default function RepositionTaskView({ task }) {
  const getGradeBadgeColor = (grade) => {
    if (grade === 'INTACT') return 'bg-green-100 text-green-700';
    if (grade === 'GRADE_1') return 'bg-yellow-100 text-yellow-700';
    if (grade === 'GRADE_2') return 'bg-orange-100 text-orange-700';
    if (grade === 'GRADE_3') return 'bg-red-100 text-red-700';
    if (grade === 'GRADE_4') return 'bg-purple-100 text-purple-700';
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gray-50 rounded-xl p-3">
            ...
            <p className="text-base font-semibold text-gray-900">{task.position?.replace(/_/g, ' ')}</p>
          </div>

          <div className="bg-gray-50 rounded-xl p-3">
            <p className="text-xs font-medium text-gray-500 uppercase mb-1">Intact or EPUAP Grade</p>
            <span className={`inline-block px-3 py-1 text-sm rounded-full font-semibold ${getGradeBadgeColor(task.intactOrEpuapGrade)}`}>
              {task.intactOrEpuapGrade?.replace(/_/g, ' ')}
            </span>
          </div>
        </div>
      ...
      {task.photoUrl && (
        <div className="bg-gray-50 rounded-xl p-3">
          ...
          <div className="mt-2">
            ...
            />
            <p className="text-sm text-gray-500 hidden">Photo: {task.photoUrl}</p>
          </div>
        </div>
      )}
