...
export async function GET(request, { params }) {
  try {
    ...
    const task = await prisma.stoolTask.findUnique({
      where: { id: taskId },
      include: {
        serviceSeeker: true,
        createdBy: {
    ...
    console.error('GET /stool-tasks/[id] error:', error);
    return NextResponse.json(
      { error: 'Failed to fetch stool task' },
      { status: 500 }
    );
  }
}

// PUT - Update stool task
export async function PUT(request, { params }) {
  try {
    ...
    const existingTask = await prisma.stoolTask.findUnique({
      where: { id: taskId },
      include: {
        serviceSeeker: true
      }
    ...
    console.error('PUT /stool-tasks/[id] error:', error);
    return NextResponse.json(
      { error: 'Failed to update stool task' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request, { params }) {
  try {
    ...
    console.error('DELETE /stool-tasks/[id] error:', error);
    return NextResponse.json(
      { error: 'Failed to delete stool task' },
      { status: 500 }
    );
  }
}
