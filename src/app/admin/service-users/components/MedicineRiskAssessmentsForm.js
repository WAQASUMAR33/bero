...
const riskTypes = ['Assessment of a Service User for Self-Medication', 'Medication'];

const selfMedicationQuestions = [
  'Understands their medication and its purposes?',
  'Understands that medication must not be given to other service users?',
  'Understands the need to keep medication locked away when not in use?',
  'Knows where their keys are safely kept?',
  'Is able to read and UNDERSTAND instructions on medicine containers?',
  'Is able to remember when to take medication?',
  'Understands the medicine dosages to be taken?',
  'Is able to open tablet bottles / containers and remove medication?',
  'Is able to "pop" tablets through blister strips to remove medication?',
  'Is able to use eyedrops / ear drops / nasal drops?',
  'Is able to use inhalers?',
  'Is able to aply creams and ointments safely and effectively?',
  'Is able to self-inject safely?',
  'Understands the need to dispose of used needles safely in "sharps" containers?',
  'Can safely handle "sharps" containers and request replacements when full?',
];

export default function MedicineRiskAssessmentsForm({ serviceSeekerId, serviceUserName, onNotification }){
  ...
  const [formData, setFormData] = useState({
    ...
    medication: '',
    whatIsRisk: '', riskBeforeIntervention: '', whoIsAtRisk: '', isHistorical: '', whatCouldHappen: '', actionToTake: '', riskAfterControls: '', summary: '', riskLevel: '', totalScore: '', staffTeam: [], conductedBy: '', office: '', sendSignoffs: false, extra: { selfMedication: {} }
  });

  const isSelfMedication = formData.riskType === riskTypes[0];
  ...
  const fetchRows = async () => {
    ...
    setLoading(true);
    try{ const token = localStorage.getItem('token'); const res = await fetch(`/api/service-seekers/${serviceSeekerId}/risk-assessments?riskType=${encodeURIComponent(riskTypes[0])}`,{ headers:{ Authorization:`Bearer ${token}` }}); const res2 = await fetch(`/api/service-seekers/${serviceSeekerId}/risk-assessments?riskType=${encodeURIComponent(riskTypes[1])}`,{ headers:{ Authorization:`Bearer ${token}` }}); const a = res.ok ? await res.json() : []; const b = res2.ok ? await res2.json() : []; setRows([...(a||[]), ...(b||[])]); }catch(e){ console.error(e); } finally{ setLoading(false); }
  };
  ...
  const openAdd = () => { setFormData({ riskType:'', lastAssessed:'', reviewFrequency:'Every six months or 26 weeks', medication:'', whatIsRisk:'', riskBeforeIntervention:'', whoIsAtRisk:'', isHistorical:'', whatCouldHappen:'', actionToTake:'', riskAfterControls:'', summary:'', riskLevel:'', totalScore:'', staffTeam: [], conductedBy:'', office:'', sendSignoffs:false, extra:{ selfMedication: {} } }); setShowModal(true); };

  const save = async () => { setSaving(true); try{ const token = localStorage.getItem('token'); const res = await fetch(`/api/service-seekers/${serviceSeekerId}/risk-assessments`,{ method:'POST', headers:{ 'Content-Type':'application/json', Authorization:`Bearer ${token}` }, body: JSON.stringify(formData) }); if(res.ok){ await fetchRows(); setShowModal(false); if(onNotification) onNotification({ show:true, message:'Risk assessment saved.', type:'success' }); } else { const err = await res.json(); if(onNotification) onNotification({ show:true, message: err.error || 'Failed to save.', type:'error' }); } }catch(e){ console.error(e); if(onNotification) onNotification({ show:true, message:'Failed to save.', type:'error' }); } finally{ setSaving(false); } };
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {isSelfMedication ? (
                  <>
                    <div className="md:col-span-2"><label className="block text-sm text-gray-600 mb-1">Medication</label><input type="text" value={formData.medication} onChange={e=>setFormData(prev=>({...prev, medication:e.target.value}))} className="w-full border rounded-lg px-3 py-2 text-gray-900" placeholder="Medication" /></div>
                    <div className="md:col-span-2"><h4 className="text-sm font-semibold text-gray-900 mt-2">Service User Risk Element</h4></div>
                    {selfMedicationQuestions.map(q => (
                      <div key={q} className="md:col-span-1">
                        <label className="block text-sm text-gray-600 mb-1">{q}</label>
                        <select value={formData.extra?.selfMedication?.[q] || ''} onChange={e=>{ setFormData(prev=>({ ...prev, extra: { ...(prev.extra||{}), selfMedication: { ...(prev.extra?.selfMedication||{}), [q]: e.target.value } } })); }} className="w-full border rounded-lg px-3 py-2 text-gray-900"><option value="">Please Select</option><option>Yes</option><option>No</option></select>
                      </div>
                    ))}
                  </>
                ) : (
