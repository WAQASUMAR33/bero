export default function OralCareTaskView({ task }) {
  ...
  const getOralCareDisplay = (type) => {
    const types = {
      BRUSHED_TEETH: '🦷 Brushed Teeth',
      DENTURE_SOAK: '💧 Denture Soak',
      DENTURE_FILLED: '🔧 Denture Filled',
      MOUTH_WASH: '🧴 Mouth Wash'
    };
    return types[type] || type;
  };

  const getAssistedDisplay = (type) => {
    return type === 'SELF_ASSISTED' ? '👤 Self Assisted' : '🤝 Assisted';
  };

  const getComplianceDisplay = (status) => {
    return status === 'COMPLETED' ? '✅ Completed' : '❌ Declined';
  };
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <p className="mt-1 text-lg font-semibold text-gray-900">
              {getOralCareDisplay(task.oralCare)}
            </p>
          </div>
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <p className="mt-1 text-lg font-semibold text-gray-900">
              {getAssistedDisplay(task.assisted)}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <p className="mt-1 text-lg font-semibold text-gray-900">
              {getComplianceDisplay(task.compliance)}
            </p>
          </div>
