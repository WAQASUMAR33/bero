...
export default function RiskAssessmentsForm({ serviceSeekerId, serviceUserName, onNotification, onRiskAssessmentChange }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const standardKeys = [
        ...
        'serviceUserOption', 'serviceUserViews', 'linkedSupportPlans'
      ];
      ...
      const hasEval = formData.evaluationRecord?.trim() || formData.evaluatorName?.trim() || formData.serviceUserViews?.trim() || formData.serviceUserOption;
      const existingEvaluations = Array.isArray(extraData.evaluations) ? extraData.evaluations : [];
      const assessmentName = formData.riskType || activeTab || 'Risk Assessment';
      const initialEvaluations = hasEval ? [{
        ...
        serviceUserOption: formData.serviceUserOption || null,
        serviceUserRead: formData.serviceUserOption === 'READ',
        serviceUserDoNotWish: formData.serviceUserOption === 'DO_NOT_WISH',
        serviceUserViews: formData.serviceUserViews?.trim() || '',
        planOrAssessmentName: assessmentName,
  ...
  const handleAddEvaluation = async () => {
    ...
    try {
      ...
      const assessmentName = viewRecord.riskType || activeTab || 'Risk Assessment';
      const newEval = {
        id: `eval_${Date.now()}`,
                ...
                <div className="md:col-span-2 pt-4 mt-2 border-t-2 border-blue-100 space-y-4">
                  ...
                  <div className="bg-gradient-to-br from-blue-50/70 to-indigo-50/40 border border-blue-200 rounded-xl p-5 shadow-xs">
                    ...
                    <p className="text-xs text-gray-600 mb-4">
                      Record the staff member's evaluation review for this assessment. All recorded evaluations remain visible permanently.
                    </p>
                    ...
                    <div>
                      ...
                      <textarea
                        rows={3}
                        placeholder="Enter staff evaluation record, review notes, risk mitigation efficacy, or necessary updates..."
                  ...
                  <div className="bg-gradient-to-br from-amber-50/70 to-orange-50/40 border border-amber-200 rounded-xl p-5 shadow-xs">
                    ...
                    <p className="text-xs text-gray-600 mb-4">
                      Record the service user's participation, choices, and views regarding this assessment.
                    </p>
                    ...
                    <div className="bg-white rounded-lg p-3.5 border border-amber-200/80 mb-4 space-y-2.5">
                      <label className="flex items-start gap-2.5 cursor-pointer select-none">
                        ...
                        <span className="text-xs font-medium text-gray-800">
                          I have read the <strong className="text-gray-900">{formData.riskType || activeTab || 'Risk Assessment'}</strong>
                        </span>
                      </label>
                ...
                {viewRecord.extra && typeof viewRecord.extra === 'object' && Object.keys(viewRecord.extra).length > 0 && (
                  <div className="space-y-3 pt-2">
                    ...
                    {Object.entries(viewRecord.extra)
                      .filter(([k]) => k !== 'evaluations' && k !== 'evaluationDate' && k !== 'evaluatorName' && k !== 'evaluationRecord' && k !== 'serviceUserOption' && k !== 'serviceUserViews' && k !== 'linkedSupportPlans')
                      .map(([k, v]) => {
              ...
              <div className="pt-6 border-t-2 border-gray-200">
                ...
                {!Array.isArray(viewRecord.extra?.evaluations) || viewRecord.extra.evaluations.length === 0 ? (
                  ...
                  <div className="space-y-3 mb-6">
                    {viewRecord.extra.evaluations.map((ev, idx) => {
                      const itemAssessmentName = ev.planOrAssessmentName || viewRecord.riskType || activeTab || 'Risk Assessment';
                      return (
                                ...
                                <div className="mb-2">
                                  {ev.serviceUserOption === 'READ' || ev.serviceUserRead ? (
                                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-semibold bg-emerald-100 text-emerald-800 border border-emerald-200">
                                      ...
                                      I have read the {itemAssessmentName}
                                    </span>
                ...
                <div className="bg-white border-2 border-blue-200 rounded-xl p-5 shadow-xs space-y-4">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Add a new evaluation review to this assessment. It will be recorded alongside existing evaluations.</p>
                  </div>
                  ...
                  <div className="bg-amber-50/50 border border-amber-200 rounded-xl p-4 space-y-3">
                    ...
                    <div className="bg-white rounded-lg p-3 border border-amber-200/80 space-y-2.5">
                      <label className="flex items-start gap-2.5 cursor-pointer select-none">
                        ...
                        />
                        <span className="text-xs font-medium text-gray-800">
                          I have read the <strong className="text-gray-900">{viewRecord?.riskType || activeTab || 'Risk Assessment'}</strong>
                        </span>
                      </label>
                      <div className="flex items-center gap-2 pl-6">
