'use server';

import { NextResponse } from 'next/server';
import { prisma } from '@/lib/prisma';
import jwt from 'jsonwebtoken';

export async function GET(request, { params }) {
  try {
    const token = request.headers.get('authorization')?.replace('Bearer ', '');
    if (!token) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    jwt.verify(token, process.env.JWT_SECRET || 'your-secret-key');

    const resolvedParams = await params;
    const serviceSeekerId = parseInt(resolvedParams.id, 10);
    if (Number.isNaN(serviceSeekerId)) return NextResponse.json({ error: 'Invalid ID' }, { status: 400 });

    const instructions = await prisma.serviceSeekerSocialVisitInstructions.findUnique({
      where: { serviceSeekerId },
    });
    
    return NextResponse.json(instructions || { instructions: '' }, { status: 200 });
  } catch (e) {
    console.error('GET social-visit-instructions error:', e);
    return NextResponse.json({ error: 'Failed to fetch instructions' }, { status: 500 });
  }
}

export async function POST(request, { params }) {
  try {
    const token = request.headers.get('authorization')?.replace('Bearer ', '');
    if (!token) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    jwt.verify(token, process.env.JWT_SECRET || 'your-secret-key');

    const resolvedParams = await params;
    const serviceSeekerId = parseInt(resolvedParams.id, 10);
    if (Number.isNaN(serviceSeekerId)) return NextResponse.json({ error: 'Invalid ID' }, { status: 400 });

    const b = await request.json();
    
    const instructions = await prisma.serviceSeekerSocialVisitInstructions.upsert({
      where: { serviceSeekerId },
      update: {
        instructions: b.instructions || null,
      },
      create: {
        serviceSeekerId,
        instructions: b.instructions || null,
      },
    });
    
    return NextResponse.json(instructions, { status: 200 });
  } catch (e) {
    console.error('POST social-visit-instructions error:', e);
    return NextResponse.json({ error: 'Failed to save instructions' }, { status: 500 });
  }
}

