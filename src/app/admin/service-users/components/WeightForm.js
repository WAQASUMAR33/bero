...
export default function WeightForm({ serviceSeekerId, onNotification }) {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/service-seekers/${serviceSeekerId}/weight-schedule-items`, {
  ...
  const saveScheduleItem = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const url = `/api/service-seekers/${serviceSeekerId}/weight-schedule-items`;
  ...
  const deleteScheduleItem = async (id) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/service-seekers/${serviceSeekerId}/weight-schedule-items?id=${id}`,
        {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` },
        }
                  ...
                  {displayRows.length === 0 ? (
                    <tr>
                      <td colSpan="6" className="py-8 text-center text-gray-500">
                        No weight schedules yet. Click the + button to add one.
                      </td>
                    </tr>
                  ) : (
            ...
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-semibold">
                  {editingId ? 'Edit Weight Schedule' : 'New Weight Task'}
                </h3>
                <button
                  type="button"
