...
# Usage: .\scripts\test-clock-in-api.ps1 -Email "your-email@example.com" -Password "your-password"
...
try {
    ...
    if ($shiftsResponse.success) {
        ...
        if ($shifts.Count -eq 0) {
            Write-Host "No shifts found. Please create a shift first." -ForegroundColor Yellow
            exit 1
        }
...
if ($firstShift.clockedIn) {
    Write-Host "Already clocked in for this shift. Clock out first to test clock-in again." -ForegroundColor Yellow
    Write-Host ""
