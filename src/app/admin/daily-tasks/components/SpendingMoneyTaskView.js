...
export default function SpendingMoneyTaskView({ task }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-6 text-center border-2 border-green-200">
        <p className="text-sm font-medium text-green-700 uppercase mb-2">Amount</p>
        <p className="text-4xl font-bold text-green-900">{formatCurrency(task.amount)}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gray-50 rounded-xl p-3">
            ...
            <p className="text-base font-semibold text-gray-900">{task.type?.replace(/_/g, ' ')}</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-gray-50 rounded-xl p-3">
            <p className="text-xs font-medium text-gray-500 uppercase mb-1">Paid Using</p>
            <div className="flex items-center space-x-2">
              <span className="text-2xl">{getPaymentIcon(task.paidUsing)}</span>
              <span className="text-base font-semibold text-gray-900">{task.paidUsing?.replace(/_/g, ' ')}</span>
            </div>
          </div>
      ...
      {task.receiptUrl && (
        <div className="bg-gray-50 rounded-xl p-3">
          <p className="text-xs font-medium text-gray-500 uppercase mb-2">Receipt</p>
          <div className="mt-2">
            <img 
              src={task.receiptUrl} 
              alt="Receipt" 
            ...
            />
            <p className="text-sm text-gray-500 hidden">Receipt: {task.receiptUrl}</p>
          </div>
        </div>
      )}
