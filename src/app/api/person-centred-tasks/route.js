...
export async function GET(request) {
  try {
    ...
    if (isCareWorker) {
      ...
      const allSchedules = await prisma.serviceSeekerPersonCentredSchedule.findMany({
    ...
    if (serviceSeekerIds.length > 0) {
      await generatePersonCentredTasksFromSchedules(serviceSeekerIds, targetDate, decoded.userId);
    }
