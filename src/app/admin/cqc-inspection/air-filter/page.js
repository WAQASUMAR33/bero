...
export default function AirFilterPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`/api/cqc-inspection/air-filter?year=${year}`, {
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center space-x-4 mb-2">
                  ...
                  <h1 className="text-3xl font-bold text-gray-900">For house keeping tasks do you check the air filter on the clothes dryer on a weekly basis?</h1>
                </div>
                <p className="text-gray-600 ml-14">Housekeeping task completion per service user by month</p>
              </div>
              ...
              <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-gray-900">
                    Housekeeping Task Completion ({serviceUsers.length} service users)
                  </h3>
                  <div className="text-sm text-gray-600">
                    Completed: <span className="font-semibold text-green-600">{grandCompleted}</span>
                    <span className="mx-1">/</span>
                    <span className="font-semibold">{grandTotal}</span> tasks
                    {grandTotal > 0 && (
                      <span className="ml-2 font-semibold text-[#224fa6]">
                        ({Math.round((grandCompleted / grandTotal) * 100)}%)
                      </span>
                    )}
                  </div>
                </div>
                    ...
                    {serviceUsers.length === 0 ? (
                      <tr>
                        <td colSpan={14} className="px-6 py-12 text-center text-gray-500">
                          No housekeeping task data available for {year}.
                        </td>
                      </tr>
                    ) : (
                      <>
