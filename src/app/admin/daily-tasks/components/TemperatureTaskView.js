...
export default function TemperatureTaskView({ task }) {
  const getTempBadgeColor = (temp) => {
    if (temp < 36) return 'bg-blue-100 text-blue-700 border-blue-300';
    if (temp >= 36 && temp <= 37.5) return 'bg-green-100 text-green-700 border-green-300';
    if (temp > 37.5 && temp <= 38.5) return 'bg-yellow-100 text-yellow-700 border-yellow-300';
    if (temp > 38.5) return 'bg-red-100 text-red-700 border-red-300';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`rounded-xl p-6 text-center border-2 ${getTempBadgeColor(task.temperatureInC)}`}>
        <p className="text-sm font-medium uppercase mb-2">Temperature</p>
        <p className="text-5xl font-bold mb-2">{task.temperatureInC}°C</p>
        <p className="text-lg font-semibold">{getTempStatus(task.temperatureInC)}</p>
      </div>
