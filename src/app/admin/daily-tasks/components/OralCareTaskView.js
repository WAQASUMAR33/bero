...
export default function OralCareTaskView({ task }) {
  return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
          <div className="bg-gray-50 rounded-xl p-3">
            ...
            <p className="text-base font-semibold text-gray-900">{new Date(task.date).toLocaleDateString()} {task.time ? `at ${task.time}` : ''}</p>
          </div>

          <div className="bg-gray-50 rounded-xl p-3">
            <p className="text-xs font-medium text-gray-500 uppercase mb-1">Oral Care Type</p>
            <p className="text-base font-semibold text-gray-900">{task.oralCare?.replace(/_/g, ' ')}</p>
          </div>

          <div className="bg-gray-50 rounded-xl p-3">
            <p className="text-xs font-medium text-gray-500 uppercase mb-1">Assistance</p>
            <p className="text-base font-semibold text-gray-900">{task.assisted?.replace(/_/g, ' ')}</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-gray-50 rounded-xl p-3">
            ...
            <span className={`inline-block px-3 py-1 text-sm rounded-full font-semibold ${task.compliance === 'COMPLETED' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              {task.compliance}
            </span>
          </div>
