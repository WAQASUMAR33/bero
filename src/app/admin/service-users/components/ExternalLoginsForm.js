      ...
      reader.onload = async (event) => {
        try {
          ...
          if (res.ok && data.success && data.fileUrl) {
            setProfileData(prev => ({ ...prev, picture: data.fileUrl }));
            if (onNotification) onNotification({ show: true, message: 'Picture uploaded.', type: 'success' });
          } else {
            ...
            <div className="p-6 bg-white space-y-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                <div className="flex-1">
                  <input type="file" accept="image/*" onChange={handlePictureChange} className="hidden" id="picture-upload" disabled={uploadingImage} />
                  ...
                  <span className="ml-2 text-sm text-gray-600">
                    {uploadingImage ? '' : (profileData.picture ? (
                      <a href={profileData.picture} target="_blank" rel="noopener noreferrer" className="text-[#224fa6] hover:underline">
                        Image uploaded
                      </a>
                    ) : 'No file chosen')}
                  </span>
