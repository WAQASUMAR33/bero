...
export async function PUT(request, { params }) {
  try {
    ...
    const existingTask = await prisma.communicationNotesTask.findUnique({
      where: { id: taskId },
      include: {
        serviceSeeker: true
      }
    ...
    if (isCareWorker) {
      // Care workers can only update: task data, notes, emotion (communication-notes tasks don't have completed field)
...
export async function DELETE(request, { params }) {
  try {
    ...
    await prisma.communicationNotesTask.delete({ where: { id: taskId } });
