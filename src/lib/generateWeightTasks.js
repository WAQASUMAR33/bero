...
/**
 * Generate weight tasks from schedules for a specific date
 ...
 */
export async function generateWeightTasksFromSchedules(serviceSeekerIds, targetDate, userId = 1) {
  try {
    ...
    // Get all active weight schedules for the service seeker(s)
    const scheduleWhere = Array.isArray(serviceSeekerIds) 
      ? { serviceSeekerId: { in: serviceSeekerIds } }
      : { serviceSeekerId: serviceSeekerIds };
    
    const schedules = await prisma.serviceSeekerWeightSchedule.findMany({
      where: scheduleWhere,
    });
    
    if (schedules.length === 0) {
      console.log(`[generateWeightTasks] No schedules found for service seeker(s): ${Array.isArray(serviceSeekerIds) ? serviceSeekerIds.join(', ') : serviceSeekerIds}`);
      return 0;
    }
    
    console.log(`[generateWeightTasks] Found ${schedules.length} schedule(s) for date ${targetDateStart.toISOString().split('T')[0]}`);
    
    // Check which tasks already exist for this date
    const existingTasks = await prisma.weightTask.findMany({
      where: {
    ...
    for (const schedule of schedules) {
      ...
      for (const timeSlot of times) {
        ...
        try {
          await prisma.weightTask.create({
            data: {
              serviceSeekerId: schedule.serviceSeekerId,
              date: targetDateStart,
              time: timeString,
              weight: 0, // Empty/unfilled - care worker will fill this
              completed: 'NO',
              emotion: 'NEUTRAL',
              createdById: userId,
              updatedById: userId,
          ...
          console.log(`[generateWeightTasks] Created task for service seeker ${schedule.serviceSeekerId} at ${timeString}`);
        } catch (error) {
          // Skip if task already exists (race condition)
          if (error.code !== 'P2002') {
            console.error('Error creating weight task from schedule:', error);
          }
        }
      }
    }
    
    console.log(`[generateWeightTasks] Created ${tasksCreated} task(s) for date ${targetDateStart.toISOString().split('T')[0]}`);
    return tasksCreated;
  } catch (error) {
    console.error('Error generating weight tasks from schedules:', error);
    return 0;
  }
}
