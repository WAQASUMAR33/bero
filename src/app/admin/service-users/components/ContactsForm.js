...
export default function ContactsForm({ serviceSeekerId, onNotification }) {
  ...
  const handleAdd = () => { resetForm(); setSelectedFile(null); setShowModal(true); };
  ...
  const handleEdit = (item) => {
    ...
    setSelectedFile(null); // Reset selection on edit open, or could try to prepopulate but file input is tricky
    setShowModal(true);
  };
                  ...
                  <div className="md:col-span-2">
                    ...
                    {formData.pictureUrl && (
                      <p className="text-xs mt-1">
                        <a href={formData.pictureUrl} target="_blank" rel="noopener noreferrer" className="text-green-600 hover:underline">
                          Image uploaded: {selectedFile?.name || 'File'}
                        </a>
                      </p>
                    )}
                  </div>
