...
/**
 * Generate Person Centred tasks from schedules for a specific date
 ...
 */
export async function generatePersonCentredTasksFromSchedules(serviceSeekerIds, targetDate, userId = 1) {
  try {
    ...
    // Get all active Person Centred schedules for the service seeker(s)
    const scheduleWhere = Array.isArray(serviceSeekerIds) 
      ? { serviceSeekerId: { in: serviceSeekerIds } }
      : { serviceSeekerId: serviceSeekerIds };
    
    const schedules = await prisma.serviceSeekerPersonCentredSchedule.findMany({
      where: scheduleWhere,
      include: {
        taskName: true,
      },
    ...
    if (schedules.length === 0) {
      console.log(`[generatePersonCentredTasks] No schedules found for service seeker(s): ${Array.isArray(serviceSeekerIds) ? serviceSeekerIds.join(', ') : serviceSeekerIds}`);
      return 0;
    }
    
    console.log(`[generatePersonCentredTasks] Found ${schedules.length} schedule(s) for date ${targetDateStart.toISOString().split('T')[0]}`);
    
    // Check which tasks already exist for this date
    const existingTasks = await prisma.personCentredTask.findMany({
      where: {
    ...
    // Create a set of existing task keys (serviceSeekerId-nameId-time) for quick lookup
    const existingTaskKeys = new Set(
      existingTasks.map(t => `${t.serviceSeekerId}-${t.nameId}-${t.time}`)
    );
    ...
    for (const schedule of schedules) {
      ...
      for (const timeSlot of times) {
        ...
        const taskKey = `${schedule.serviceSeekerId}-${schedule.nameId}-${timeString}`;
        
        // Skip if task already exists (same service seeker, name, and time)
        if (existingTaskKeys.has(taskKey)) {
          continue;
        }
        
        try {
          await prisma.personCentredTask.create({
            data: {
              serviceSeekerId: schedule.serviceSeekerId,
              date: targetDateStart,
              time: timeString,
              nameId: schedule.nameId, // This creates separate tasks for each name
              notes: schedule.directions || null,
          ...
          console.log(`[generatePersonCentredTasks] Created task for service seeker ${schedule.serviceSeekerId} with name "${schedule.taskName.name}" at ${timeString}`);
        } catch (error) {
          // Skip if task already exists (race condition)
          if (error.code !== 'P2002') {
            console.error('Error creating Person Centred task from schedule:', error);
          }
        }
      }
    }
    
    console.log(`[generatePersonCentredTasks] Created ${tasksCreated} task(s) for date ${targetDateStart.toISOString().split('T')[0]}`);
    return tasksCreated;
  } catch (error) {
    console.error('Error generating Person Centred tasks from schedules:', error);
    return 0;
  }
}
