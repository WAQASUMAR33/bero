...
export default function StoolCheckForm({ serviceSeekerId, onNotification }) {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/service-seekers/${serviceSeekerId}/stool-schedule-items`, {
  ...
  const openAdd = () => {
    setEditingId(null);
    setFormData({ frequency: '', team: 'All', times: [] });
  ...
  const saveScheduleItem = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const url = `/api/service-seekers/${serviceSeekerId}/stool-schedule-items`;
  ...
  const deleteScheduleItem = async (id) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/service-seekers/${serviceSeekerId}/stool-schedule-items?id=${id}`,
        {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden mb-8 border-t-4 border-orange-500">
      ...
      <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white px-6 py-4">
        <h2 className="text-xl font-semibold">Stool Checks</h2>
      </div>
                  ...
                  {displayRows.length === 0 ? (
                    <tr>
                      <td colSpan="6" className="py-8 text-center text-gray-500">
                        No stool check schedules yet. Click the + button to add one.
                      </td>
                    </tr>
                  ) : (
            ...
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-semibold">
                  {editingId ? 'Edit Stool Check Schedule' : 'New Stool Check Schedule'}
                </h3>
                <button
                  type="button"
