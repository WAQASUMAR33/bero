...
export async function GET(request, { params }) {
  try {
    ...
    const schedule = await prisma.serviceSeekerBathingSchedule.findUnique({
      ...
      include: { scheduleItems: { orderBy: { createdAt: 'desc' } } },
    });

    return NextResponse.json(schedule?.scheduleItems || [], { status: 200 });
  } catch (e) {
    console.error('GET bathing-schedule-items error:', e);
...
export async function POST(request, { params }) {
  try {
    ...
    let schedule = await prisma.serviceSeekerBathingSchedule.findUnique({
      where: { serviceSeekerId },
    });

    if (!schedule) {
      schedule = await prisma.serviceSeekerBathingSchedule.create({
        data: { serviceSeekerId },
      });
    }

    const created = await prisma.serviceSeekerBathingScheduleItem.create({
      data: {
        scheduleId: schedule.id,
        bathingType: b.bathingType || '',
    ...
    // Create corresponding bathing tasks if needed
    if (b.createTasks && Array.isArray(b.times) && b.times.length > 0) {
      const today = new Date();
      const bathingTypeEnum = b.bathingTypeEnum || b.bathingType.toUpperCase().replace(/\s+/g, '_').replace(/-/g, '_');
        ...
        if (isDaily || selectedDays.includes(dayName)) {
          for (const timeSlot of b.times) {
            ...
            if (isDaily || timeSlot.day === dayName || !timeSlot.day) {
              try {
                await prisma.bathingTask.create({
                  data: {
                    serviceSeekerId,
                    date: date,
                    time: `${timeSlot.hour || '00'}:${timeSlot.minute || '00'}`,
                    bathingType: bathingTypeEnum,
                ...
                if (taskError.code !== 'P2002') {
                  console.error('Error creating bathing task:', taskError);
                }
              }
            }
          }
    ...
    console.error('POST bathing-schedule-items error:', e);
...
export async function PUT(request, { params }) {
  try {
    ...
    const updated = await prisma.serviceSeekerBathingScheduleItem.update({
      where: { id },
      data: {
        bathingType: b.bathingType || '',
        frequency: b.frequency || '',
    ...
    console.error('PUT bathing-schedule-items error:', e);
...
export async function DELETE(request, { params }) {
  try {
    ...
    await prisma.serviceSeekerBathingScheduleItem.delete({ where: { id } });
    return NextResponse.json({ message: 'Deleted' }, { status: 200 });
  } catch (e) {
    console.error('DELETE bathing-schedule-items error:', e);
