...
export default function OxygenTaskForm({ 
  formData, 
  setFormData, 
  serviceSeekers, 
  onSubmit, 
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      {/* Quantity */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">Oxygen Quantity (L/min) *</label>
        <input
          type="text"
          required
          value={formData.quantity}
          onChange={(e) => setFormData({ ...formData, quantity: e.target.value })}
          placeholder="e.g., 2 L/min, 3 L/min"
          className="w-full border border-gray-300 rounded-lg px-3 py-2 text-gray-900 focus:ring-2 focus:ring-[#224fa6] focus:border-transparent"
        />
      </div>
