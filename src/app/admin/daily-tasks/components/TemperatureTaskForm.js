...
export default function TemperatureTaskForm({ 
  formData, 
  setFormData, 
  serviceSeekers, 
  onSubmit, 
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      {/* Temperature */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">Temperature in °C *</label>
        <input
          type="number"
          step="0.1"
          min="30"
          max="45"
          required
          value={formData.temperatureInC}
          onChange={(e) => setFormData({ ...formData, temperatureInC: e.target.value })}
          placeholder="36.5"
          className="w-full border border-gray-300 rounded-lg px-3 py-2 text-gray-900 focus:ring-2 focus:ring-[#224fa6] focus:border-transparent"
        />
      </div>
