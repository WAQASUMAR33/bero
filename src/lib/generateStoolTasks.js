...
/**
 * Generate stool tasks from schedules for a specific date
 ...
 */
export async function generateStoolTasksFromSchedules(serviceSeekerIds, targetDate, userId = 1) {
  try {
    ...
    // Get all active stool schedules for the service seeker(s)
    const scheduleWhere = Array.isArray(serviceSeekerIds) 
      ? { serviceSeekerId: { in: serviceSeekerIds } }
      : { serviceSeekerId: serviceSeekerIds };
    
    const schedules = await prisma.serviceSeekerStoolSchedule.findMany({
      where: scheduleWhere,
    });
    
    if (schedules.length === 0) {
      console.log(`[generateStoolTasks] No schedules found for service seeker(s): ${Array.isArray(serviceSeekerIds) ? serviceSeekerIds.join(', ') : serviceSeekerIds}`);
      return 0;
    }
    
    console.log(`[generateStoolTasks] Found ${schedules.length} schedule(s) for date ${targetDateStart.toISOString().split('T')[0]}`);
    
    // Check which tasks already exist for this date
    const existingTasks = await prisma.stoolTask.findMany({
      where: {
    ...
    for (const schedule of schedules) {
      ...
      for (const timeSlot of times) {
        ...
        try {
          ...
          console.log(`[generateStoolTasks] Created task for service seeker ${schedule.serviceSeekerId} at ${timeString}`);
        } catch (error) {
          // Skip if task already exists (race condition)
          if (error.code !== 'P2002') {
            console.error('Error creating stool task from schedule:', error);
          }
        }
      }
    }
    
    console.log(`[generateStoolTasks] Created ${tasksCreated} task(s) for date ${targetDateStart.toISOString().split('T')[0]}`);
    return tasksCreated;
  } catch (error) {
    console.error('Error generating stool tasks from schedules:', error);
    return 0;
  }
}
