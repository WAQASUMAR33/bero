...
export async function GET(request, { params }) {
  try {
    ...
    const scheduleItems = await prisma.serviceSeekerMuacSchedule.findMany({
    ...
    console.error('GET muac-schedule-items error:', e);
...
export async function POST(request, { params }) {
  try {
    ...
    const created = await prisma.serviceSeekerMuacSchedule.create({
      data: {
        serviceSeekerId,
        times: b.times,
    ...
    console.error('POST muac-schedule-items error:', e);
...
export async function PUT(request, { params }) {
  try {
    ...
    const updated = await prisma.serviceSeekerMuacSchedule.update({
      where: { id },
      data: {
        times: b.times,
        frequency: b.frequency,
    ...
    console.error('PUT muac-schedule-items error:', e);
...
export async function DELETE(request, { params }) {
  try {
    ...
    await prisma.serviceSeekerMuacSchedule.delete({ where: { id } });
    return NextResponse.json({ message: 'Deleted' }, { status: 200 });
  } catch (e) {
    console.error('DELETE muac-schedule-items error:', e);
