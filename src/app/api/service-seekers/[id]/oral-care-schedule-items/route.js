...
export async function GET(request, { params }) {
  try {
    ...
    const scheduleItems = await prisma.serviceSeekerOralCareSchedule.findMany({
    ...
    console.error('GET oral-care-schedule-items error:', e);
...
export async function POST(request, { params }) {
  try {
    ...
    const created = await prisma.serviceSeekerOralCareSchedule.create({
      data: {
        serviceSeekerId,
        toothpasteMouthwashBrand: b.toothpasteMouthwashBrand || null,
    ...
    // Create corresponding oral care tasks if needed
    if (b.createTasks && Array.isArray(b.times) && b.times.length > 0) {
      ...
      // For oral care, times are typically just hour:minute without day specification
        ...
        if (isDaily || selectedDays.includes(dayName)) {
          for (const timeSlot of b.times) {
            ...
            if (isDaily || timeSlot.day === dayName || !timeSlot.day) {
              try {
                await prisma.oralCareTask.create({
                  data: {
                    serviceSeekerId,
                    date: date,
                    time: `${timeSlot.hour || '00'}:${timeSlot.minute || '00'}`,
                    oralCare: 'BRUSHED_TEETH', // Default oral care type
                    assisted: 'ASSISTED', // Default assisted type
                    notes: b.directions || null,
                ...
                if (taskError.code !== 'P2002') {
                  console.error('Error creating oral care task:', taskError);
                }
              }
            }
          }
    ...
    console.error('POST oral-care-schedule-items error:', e);
...
export async function PUT(request, { params }) {
  try {
    ...
    const updated = await prisma.serviceSeekerOralCareSchedule.update({
      where: { id },
      data: {
        toothpasteMouthwashBrand: b.toothpasteMouthwashBrand || null,
        directions: b.directions || null,
    ...
    console.error('PUT oral-care-schedule-items error:', e);
...
export async function DELETE(request, { params }) {
  try {
    ...
    await prisma.serviceSeekerOralCareSchedule.delete({ where: { id } });
    return NextResponse.json({ message: 'Deleted' }, { status: 200 });
  } catch (e) {
    console.error('DELETE oral-care-schedule-items error:', e);
